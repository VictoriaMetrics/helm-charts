...
![Version](https://img.shields.io/badge/0.20.1-gray?logo=Helm&labelColor=gray&link=https%3A%2F%2Fdocs.victoriametrics.com%2Fhelm%2Fvictoria-logs-cluster%2Fchangelog%2F%230201)
...
Note: this chart installs VictoriaMetrics cluster components such as vlinsert, vlselect and vlstorage. It doesn't configure log ingestion.
...
Test the installation with command:
  ...
  - For HTTPS repository
    ...
    ```console
    helm install vmc vm/victoria-logs-cluster -f values.yaml -n NAMESPACE --debug --dry-run
    ```

  - For OCI repository

    ```console
    helm install vmc oci://ghcr.io/victoriametrics/helm-charts/victoria-logs-cluster -f values.yaml -n NAMESPACE --debug --dry-run
    ```
...
Install chart with command:
  ...
  - For HTTPS repository
    ...
    ```console
    helm install vmc vm/victoria-logs-cluster -f values.yaml -n NAMESPACE
    ```

  - For OCI repository

    ```console
    helm install vmc oci://ghcr.io/victoriametrics/helm-charts/victoria-logs-cluster -f values.yaml -n NAMESPACE
    ```
