...
*/}}
{{- define "vm.probe.readiness" -}}
{{- $app := .app -}}
{{- $pathPrefix := (index  $app.extraArgs "http.pathPrefix" | default "" | trimSuffix "/") -}}
{{- $probeScheme := ternary "HTTPS" "HTTP" ($app.extraArgs.tls | default false) -}}
{{- $ports := $app.ports | default dict -}}
{{- $port := $ports.name | default "http" -}}
{{- $probeType := "httpGet" -}}
{{- $defaultProbe := (dict $probeType (dict "path" (printf "%s/health" $pathPrefix) "scheme" $probeScheme "port" $port)) -}}
{{- $probe := mergeOverwrite $defaultProbe $app.probe.readiness -}}
{{- range $paramName, $paramValue := $probe -}}
  {{- if and (kindIs "map" $paramValue) (ne $paramName $probeType) -}}
    {{- $_ := unset $probe $probeType }}
  {{- end -}}
{{- end -}}
{{- toYaml $probe -}}
{{- end -}}
...
*/}}
{{- define "vm.probe.liveness" -}}
{{- $app := .app -}}
{{- $ports := $app.ports | default dict -}}
{{- $port := $ports.name | default "http" -}}
{{- $probeType := "tcpSocket" -}}
{{- $defaultProbe := (dict $probeType (dict "port" $port)) -}}
{{- $probe := mergeOverwrite $defaultProbe $app.probe.liveness -}}
{{- range $paramName, $paramValue := $probe -}}
  {{- if and (kindIs "map" $paramValue) (ne $paramName $probeType) -}}
    {{- $_ := unset $probe $probeType }}
  {{- end -}}
