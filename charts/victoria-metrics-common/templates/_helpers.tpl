...
{{- /*
vm.cr.component.spec builds the CommonAppsParams portion of an operator CR spec
from a component values dict passed as .comp.

Direct-mapped fields (same name in chart values and CR spec):
  replicaCount, image, resources, tolerations, affinity, nodeSelector,
  topologySpreadConstraints, priorityClassName, schedulerName, runtimeClassName,
  hostAliases, imagePullSecrets, initContainers, extraArgs

Renamed fields (chart value → CR field):
  extraContainers → containers
  extraVolumes    → volumes
  extraVolumeMounts → volumeMounts
  env             → extraEnvs
  envFrom         → extraEnvsFrom

Composed fields:
  podLabels + podAnnotations          → podMetadata.{labels,annotations}
  podSecurityContext + securityContext → securityContext (merged, "enabled" key omitted)
  horizontalPodAutoscaler (if enabled) → hpa (enabled key omitted)
*/ -}}
{{- define "vm.cr.component.spec" -}}
  {{- $comp := .comp -}}
  {{- $spec := dict -}}

  {{- /* Direct mappings */ -}}
  {{- if (kindIs "float64" (index $comp "replicaCount")) }}
    {{- $_ := set $spec "replicaCount" $comp.replicaCount -}}
  {{- end -}}
  {{- with $comp.image }}{{- $_ := set $spec "image" . }}{{- end -}}
  {{- with $comp.resources }}{{- $_ := set $spec "resources" . }}{{- end -}}
  {{- with $comp.tolerations }}{{- $_ := set $spec "tolerations" . }}{{- end -}}
  {{- with $comp.affinity }}{{- $_ := set $spec "affinity" . }}{{- end -}}
  {{- with $comp.nodeSelector }}{{- $_ := set $spec "nodeSelector" . }}{{- end -}}
  {{- with $comp.topologySpreadConstraints }}{{- $_ := set $spec "topologySpreadConstraints" . }}{{- end -}}
  {{- with $comp.priorityClassName }}{{- $_ := set $spec "priorityClassName" . }}{{- end -}}
  {{- with $comp.schedulerName }}{{- $_ := set $spec "schedulerName" . }}{{- end -}}
  {{- with $comp.runtimeClassName }}{{- $_ := set $spec "runtimeClassName" . }}{{- end -}}
  {{- with $comp.hostAliases }}{{- $_ := set $spec "hostAliases" . }}{{- end -}}
  {{- with $comp.imagePullSecrets }}{{- $_ := set $spec "imagePullSecrets" . }}{{- end -}}
  {{- with $comp.initContainers }}{{- $_ := set $spec "initContainers" . }}{{- end -}}
  {{- with $comp.extraArgs }}{{- $_ := set $spec "extraArgs" . }}{{- end -}}

  {{- /* Renamed fields */ -}}
  {{- with $comp.extraContainers }}{{- $_ := set $spec "containers" . }}{{- end -}}
  {{- with $comp.extraVolumes }}{{- $_ := set $spec "volumes" . }}{{- end -}}
  {{- with $comp.extraVolumeMounts }}{{- $_ := set $spec "volumeMounts" . }}{{- end -}}
  {{- with $comp.env }}{{- $_ := set $spec "extraEnvs" . }}{{- end -}}
  {{- with $comp.envFrom }}{{- $_ := set $spec "extraEnvsFrom" . }}{{- end -}}

  {{- /* Pod metadata */ -}}
  {{- $podMeta := dict -}}
  {{- with $comp.podLabels }}{{- $_ := set $podMeta "labels" . }}{{- end -}}
  {{- with $comp.podAnnotations }}{{- $_ := set $podMeta "annotations" . }}{{- end -}}
  {{- if $podMeta }}{{- $_ := set $spec "podMetadata" $podMeta }}{{- end -}}

  {{- /* Security context: merge pod-level and container-level into one dict */ -}}
  {{- $sc := dict -}}
  {{- with $comp.podSecurityContext -}}
    {{- if .enabled }}{{- $sc = mergeOverwrite $sc (omit . "enabled") }}{{- end -}}
  {{- end -}}
  {{- with $comp.securityContext -}}
    {{- if .enabled }}{{- $sc = mergeOverwrite $sc (omit . "enabled") }}{{- end -}}
  {{- end -}}
  {{- if $sc }}{{- $_ := set $spec "securityContext" $sc }}{{- end -}}

  {{- /* HPA */ -}}
  {{- with $comp.horizontalPodAutoscaler -}}
    {{- if .enabled }}{{- $_ := set $spec "hpa" (omit . "enabled") }}{{- end -}}
  {{- end -}}
